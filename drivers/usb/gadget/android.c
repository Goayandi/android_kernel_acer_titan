			    ...
			    const char *buff, size_t size)
{
	...
	if (enabled && !dev->enabled) {
		...
		//solved the problem of device¡®s kernel port was same when we need to scan different device port via meta mode  //begin
		...
		//solved the problem of device¡®s kernel port was same when we need to scan different device port via meta mode  //end
		list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
			if (f->enable)
				f->enable(f);
		}
