...
unsigned int PMIC_IMM_GetOneChannelValue(pmic_adc_ch_list_enum dwChannel, int deCount, int trimd)
{
	...
	case 2:
		...
		adc_result = (adc_result_temp * r_val_temp * VOLTAGE_FULL_RANGE) / ADC_PRECISE;
		printk("lwj-Vchr: adc_result=%d,r_val_tmp=%d,R1=%d,R2=%d,adc_result_t=%d,VOLfullrange=%d,ADC_PRECISE=%d\n",adc_result,r_val_temp,R_CHARGER_1,R_CHARGER_2,adc_result_temp,VOLTAGE_FULL_RANGE,ADC_PRECISE);
