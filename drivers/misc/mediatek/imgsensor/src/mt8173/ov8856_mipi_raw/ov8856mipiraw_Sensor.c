...
struct otp_struct otp_buf[512];  //huangjianlong add 170722
...
static int apply_otp(struct otp_struct *otp_ptr)
{
		...
		if((*otp_ptr).flag & 0x40){
			...
			LOG_INF("wangchao--------Base_gain = 0x%x\n",Base_gain);
			...
			LOG_INF("wangchao--------R_gain = 0x%x,0x5019 = 0x%x,0x501a = 0x%x\n",R_gain,read_cmos_sensor(0x5019),read_cmos_sensor(0x501a));
			if (G_gain > 0x400) {
				write_cmos_sensor(0x501b, G_gain >> 8);
				write_cmos_sensor(0x501c, G_gain & 0x00ff);
			}
			LOG_INF("wangchao--------G_gain = 0x%x,0x501b = 0x%x,0x501c = 0x%x\n",G_gain,read_cmos_sensor(0x501b),read_cmos_sensor(0x501c));
			if (B_gain > 0x400) {
				write_cmos_sensor(0x501d, B_gain >> 8);
				write_cmos_sensor(0x501e, B_gain & 0x00ff);
			}
			LOG_INF("wangchao--------B_gain = 0x%x,0x501d = 0x%x,0x501e = 0x%x\n",B_gain,read_cmos_sensor(0x501d),read_cmos_sensor(0x501e));
		}
		...
		if((*otp_ptr).flag & 0x10){
			...
			for(i=0;i<240;i++)
			{
				...
				LOG_INF("wangchao -------------- read 0x5900 + %d------0x%x!\n",i,read_cmos_sensor(0x5900+i));
			}
		}
		write_cmos_sensor(0x0100,0x00);
		return (*otp_ptr).flag;
...
{
	int otp_flag, otp_base, temp=0, i;
	int temp1;
	int checksum2 = 0;
	
	write_cmos_sensor(0x0100,0x01);
	// Pre-processing   set 0x5001[3] to "0"
	temp1 = read_cmos_sensor(0x5001);
	write_cmos_sensor(0x5001,(0x00 & 0x08) | (temp1 & (~0x08)));	// disable OTP_DPC
	LOG_INF("wangchao read 0x5001 reg:0x%x",read_cmos_sensor(5001));
	...
	// OTP info and awb calibration data
	otp_flag = read_cmos_sensor(0x7010);
	otp_base = 0;
	if ((otp_flag & 0xc0) == 0x40) {
		otp_base = 0x7011;	// base address of info group 1
		LOG_INF("wangchao  group 1!\n");
	} else if ((otp_flag & 0x30) == 0x10) {
		otp_base = 0x7019;	// base address of info group 2
		LOG_INF("wangchao group 2!\n");
	}
	LOG_INF("wangchao info and awb------otp_flag = 0x%x,otp_base = 0x%x\n",otp_flag,otp_base);
	if (otp_base != 0) {
		(*otp_ptr).flag = 0xC0;	// valid info and awb in OTP
		(*otp_ptr).module_id = read_cmos_sensor(otp_base);
		(*otp_ptr).lens_id = read_cmos_sensor(otp_base + 1);
		(*otp_ptr).production_year = read_cmos_sensor(otp_base + 2);
		(*otp_ptr).production_month = read_cmos_sensor(otp_base + 3);
		(*otp_ptr).production_day = read_cmos_sensor(otp_base + 4);
		temp = read_cmos_sensor(otp_base + 7);
		(*otp_ptr).rg_ratio = (read_cmos_sensor(otp_base + 5)<<2) + ((temp>>6)& 0x03);
		(*otp_ptr).bg_ratio = (read_cmos_sensor(otp_base + 6)<<2) + ((temp>>4)& 0x03);
	} else {
		(*otp_ptr).flag = 0x00;
		(*otp_ptr).module_id = 0x0;
		(*otp_ptr).lens_id = 0;
...
/*OTP Info log*/
		LOG_INF("wangchao--------(*otp_ptr).flag = 0x%x\n",(*otp_ptr).flag);
		LOG_INF("wangchao--------(*otp_ptr).module_id = 0x%x\n",(*otp_ptr).module_id);
		LOG_INF("wangchao--------(*otp_ptr).lens_id = 0x%x\n",(*otp_ptr).lens_id);
		LOG_INF("wangchao--------(*otp_ptr).production_year = 0x%x\n",(*otp_ptr).production_year);
		LOG_INF("wangchao--------(*otp_ptr).production_month = 0x%x\n",(*otp_ptr).production_month);
		LOG_INF("wangchao--------(*otp_ptr).production_day = 0x%x\n",(*otp_ptr).production_day);
		LOG_INF("wangchao--------temp = 0x%x\n",temp);
		LOG_INF("wangchao--------(*otp_ptr).rg_ratio = 0x%x\n",(*otp_ptr).rg_ratio);
		LOG_INF("wangchao--------(*otp_ptr).bg_ratio = 0x%x\n",(*otp_ptr).bg_ratio);
/*OTP Info log*/
	// OTP VCM Calibration
	...
	LOG_INF("wangchao vcm------otp_flag = 0x%x,otp_base = 0x%x\n",otp_flag,otp_base);
	
	if (otp_base != 0) {
		(*otp_ptr).flag |= 0x20;
		temp = read_cmos_sensor(otp_base + 2);
		(*otp_ptr).VCM_start = (read_cmos_sensor(otp_base) << 2) | ((temp >> 6) & 0x03);
		(*otp_ptr).VCM_end = (read_cmos_sensor(otp_base + 1) << 2) + ((temp >> 4) & 0x03);
	...
	if ((otp_flag & 0xc0) == 0x40) {
		otp_base = 0x7029;	// base address of VCM Calibration group 1
	} else if ((otp_flag & 0x30) == 0x10) {
		otp_base = 0x711a;	// base address of VCM Calibration group 2
	} 
	LOG_INF("wangchao lens shading------otp_flag = 0x%x,otp_base = 0x%x\n",otp_flag,otp_base);
	if (otp_base != 0) {
		for(i=0;i<240;i++){
			(*otp_ptr).lenc[i] = read_cmos_sensor(otp_base + i);
			checksum2 += (*otp_ptr).lenc[i];
		}
		checksum2 = (checksum2)%255 + 1;
		(*otp_ptr).checksum = read_cmos_sensor(otp_base + 240); //7119?
		LOG_INF("wangchao--------checksum = %d,checksum2 = %d\n",(*otp_ptr).checksum,checksum2);
		if((*otp_ptr).checksum == checksum2)
			(*otp_ptr).flag |= 0x10;
		
		for(i=0;i<240;i++){
			LOG_INF("wangchao --------- lenc[%d] = 0x%x",i, (*otp_ptr).lenc[i]);
		}
	...
	write_cmos_sensor(0x5001, (temp | (0x08)));	// set 0x5001[3] to 1
	LOG_INF("wangchao------ close read 0x5001 reg:0x%x",read_cmos_sensor(5001));
	write_cmos_sensor(0x0100,0x00);
	return (*otp_ptr).flag;
}
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				//Neostra huangjianlong 20170722
				{
				  struct otp_struct *otp_ptr = (struct otp_struct *)kzalloc(sizeof(struct otp_struct), GFP_KERNEL);
				  read_otp_info(otp_ptr);	
				  memcpy(otp_buf, otp_ptr, sizeof(struct otp_struct));
				  kfree(otp_ptr);
				}
				return ERROR_NONE;
			}	
...
//	mdelay(30);
	 #ifdef OV8856_OTP_DEBUG
	 {
	        //Neostra huangjianlong del 20170722
	 		//struct otp_struct *otp_ptr = (struct otp_struct *)kzalloc(sizeof(struct otp_struct), GFP_KERNEL);
			//LOG_INF("otp_debug start\n");
