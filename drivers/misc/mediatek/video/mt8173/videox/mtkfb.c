...
#include <mt-plat/neostra_hw_info.h> //Neostar huangjianlong 20170320
#endif
#define ALIGN_TO(x, n)  \
	(((x) + ((n) - 1)) & ~((n) - 1))
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	//Neostra huangjianlong add 20170320
#ifdef CONFIG_NEOSTRA_HW_INFO_NODE	
    neostra_add_hw_info("lcm", strlen(mtkfb_lcm_name) > 0? mtkfb_lcm_name:"no-found-lcm", 0);
#endif
	MSG_FUNC_LEAVE();
	return 0;
