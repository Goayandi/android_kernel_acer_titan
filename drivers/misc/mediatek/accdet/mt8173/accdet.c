...
//--add by lwj for woofer control,0505--{
int woofer_en=0;
int woofer_gpio_enable(int enable);
u8 enable_flag=0;

static int gpiopin;

int my_woofer_state = 1;//taosong add contrl for woofer power 20170515
...
/*Neostra taosong add for debounce func start 20170514*/
#if 0
	...
	return IRQ_HANDLED;
         ...
         queue_work(accdet_eint_workqueue, &accdet_eint_work);
         return IRQ_HANDLED;
/*Neostra taosong add for debounce func end 20170514*/
}
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	accdet_eint_type = IRQ_TYPE_LEVEL_LOW; /*Neostra huangjianlong mod 20170509*/
...
/*Neostra taosong add for debounce func start 20170514*/
		...
		gpio_set_debounce(gpiopin, headsetdebounce);
/*Neostra taosong add for debounce func end 20170514*/
		ret = request_irq(accdet_irq, accdet_eint_func, IRQ_TYPE_LEVEL_LOW, "ACCDET-eint", NULL);//Neostra huangjianlong mod 20170509
		if (ret > 0)
...
void accdet_get_dts_data(void)
{
	...
	if (node) {
		...
		of_property_read_u32(node, "eint-debounce", &accdet_dts_data.eint_debounce);/*Neostra taosong add for debounce func 20170514*/
		#ifdef FOUR_KEY_HEADSET
...
//taosong add contrl for woofer power 20170515 start
static ssize_t store_woofer_contrl_pwr(struct device_driver *ddri, const char *buf, size_t count)
{
	int value;
	int ret;
	...
	if((1 == !!value) && (0 == my_woofer_state)){
		my_woofer_state = 1;
		woofer_gpio_enable(1);		
	}
	
	if((0 == value) && (1 == my_woofer_state)){
		my_woofer_state = 0;
		woofer_gpio_enable(0);
		
	}

	return count;
}
static ssize_t show_woofer_contrl_pwr(struct device_driver *ddri, char *buf)
{
	return sprintf(buf, "%d\n", my_woofer_state);
}

static DRIVER_ATTR(woofer_contrl_pwr, S_IWUSR | S_IRUGO, show_woofer_contrl_pwr, store_woofer_contrl_pwr);
//taosong add contrl for woofer power 20170515 end
/*----------------------------------------------------------------------------*/
...
static struct driver_attribute *accdet_attr_list[] = {
	...
	&driver_attr_woofer_contrl_pwr,////taosong add contrl for woofer power 20170515
...
int woofer_gpio_enable(int enable)
{
	...
	if (gpio_is_valid(woofer_en)){
		if(enable){
			gpio_direction_output(woofer_en, 1);
		}else{
			gpio_direction_output(woofer_en, 0);
		}
	}
	return 0;
}
...
//--add by lwj for woofer control,0505--{
	woofer_en = of_get_named_gpio(pdev->of_node, "woofer-gpio-en", 0);
	if (gpio_is_valid(woofer_en)) {

		printk("lwj-woofer-gpio:\"%s\"(%d)\n", "woofer-gpio-en", woofer_en);

		ret = devm_gpio_request(pdev, woofer_en, "woofer-gpio-en");
	} else {
		printk("lwj-woofer-gpio: \"%s\"(%d) is %s !!\n", "woofer-gpio-en", woofer_en,
				(woofer_en == (-ENOENT) ? "non-used" : "invalid"));
	}
