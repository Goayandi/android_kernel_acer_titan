...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_PMIC:
		//Neostra huangjianlong 20170515
		if(g_boot_red_on == true && cust->data == MT65XX_LED_PMIC_NLED_ISINK0) {  //when boot turn on red && start to turn on green led
			g_boot_red_on = false;
			mt_brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK1, 0, 0); //red off
		}
