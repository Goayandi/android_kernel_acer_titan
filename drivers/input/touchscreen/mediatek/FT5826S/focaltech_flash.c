...
{
	#include "Acer_neostra_A10H2_FT5826S_V87_D04_20170426_app.i" //Neostra liudongke modify for update pingbo TP firmware 20170227
};

//Begin Neostra huangxiaohui add  20160726
static unsigned char TAIGUAN_FW[] = 
{
      #include "Acer_Titan_Neostra_A10H2_TaiGuan1351_FT5826S_V56_D04_20170831_app.i"
};
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if ( (uc_tp_fm_ver < uc_host_fm_ver) || (uc_tp_vendor_id ==0x00) )//Neostra huangxiaohui forced refresh taiguan TP firmware when uc_tp_vendor_id ==0x00 20170512
	{
	    upmu_set_rg_pwrkey_rst_en(0x00);//Add Neostra huangxiaohui for:disable long press powerkey operation,when TP FW update. 20170417
		msleep(100);
        ...
        else//Neostra huangxiaohui forced refresh taiguan TP firmware when uc_tp_vendor_id ==0x00 20170512
        {
            printk("--->>hxh forced refresh taiguan TP firmware !\n");
            i_ret = taiguan_fw_upgrade_with_i_file(client);
        }
        upmu_set_rg_pwrkey_rst_en(0x01);//Add Neostra huangxiaohui for:enable long press powerkey operation,when TP FW update finish. 20170417
         // End Neostra huangxiaohui mod  20160726
		if (i_ret == 0)	
		{
			msleep(300);
