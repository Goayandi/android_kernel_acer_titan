...
struct device_node * node ;    //Neostra huangjianlong 20170506
static unsigned int  key_irq;
unsigned int key_eint_type = IRQ_TYPE_LEVEL_LOW;
static struct workqueue_struct *key_eint_workqueue;
static struct work_struct key_eint_work;
static int gpiopin;
...
//Neostra huangjianlong 2070506
static void key_eint_work_callback(struct work_struct *work)
{
		//printk("[huang] key_eint_work_callback: gpiopin=%d, debounce=%d\n", gpiopin, debounce);

		if(key_eint_type == IRQ_TYPE_LEVEL_LOW) {
			key_eint_type = IRQ_TYPE_LEVEL_HIGH;
			input_report_key(kpd_input_dev, KEY_F24, 1);
			input_sync(kpd_input_dev);
		} else {
			key_eint_type = IRQ_TYPE_LEVEL_LOW;
			input_report_key(kpd_input_dev, KEY_F24, 0);
			input_sync(kpd_input_dev);			
		}
		
		irq_set_irq_type(key_irq, key_eint_type);
		enable_irq(key_irq);
}
...
static irqreturn_t kpd_irq_handler_2(int irq, void *dev_id)
{
	...
	queue_work(key_eint_workqueue, &key_eint_work);
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	//Neostra huangjianlong 20170506
	int ret;
...
#endif
	__set_bit(KEY_F24, kpd_input_dev->keybit);
	...
	//Neostra huangjianlong 20170506
	key_eint_type = IRQ_TYPE_LEVEL_LOW;
	key_eint_workqueue = create_singlethread_workqueue("key-eint");
	INIT_WORK(&key_eint_work, key_eint_work_callback);
	
	node = of_find_compatible_node(NULL,NULL,"mediatek, key-eint");
	if(node){
		of_property_read_u32(node, "eint-debounce", &debounce);	
		
		gpiopin = of_get_named_gpio(node, "dts-gpio", 0);
		printk("[neostra] find dts-gpio: %d\n", gpiopin);
		if (gpiopin < 0)
				printk("[neostra] not find dts-gpio\n");
		ret = gpio_request(gpiopin, "dts-gpio");
		if (ret)
		...
		ret = request_irq(key_irq, kpd_irq_handler_2, IRQ_TYPE_LEVEL_LOW , KPD_NAME, NULL);
	}	
#ifdef CONFIG_MTK_MRDUMP_KEY
