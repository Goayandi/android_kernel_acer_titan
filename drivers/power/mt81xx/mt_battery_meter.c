...
int BattVoltToTemp(int dwVolt)
{
	...
	printk("lwj--BattVoltToTemp,Vbat_on=%d,TRes_temp=%d,TRes=%d\n",dwVolt,(int)(TRes_temp&0xffff),(int)(TRes&0xffff));
...
/*
	printk("lwj--[force_get_tbat] %d,%d,%d,%d,%d,%d\n",
		 bat_temperature_volt_temp, bat_temperature_volt, fg_current_state, fg_current_temp,
		 fg_r_value, bat_temperature_val);
*/
...
s32 fgauge_get_Q_max_high_current_by_current(s32 i_current, s16 val_temp)
{
	...
	for (iIndex = 0; iIndex < saddles - 1; iIndex++) {
		...
		if (OCV_temp - V_drop < threshold) {//threshold=SYSTEM_OFF_VOLTAGE=3.5v
			if (iIndex <= 1)
				ret_Q_max = p_bat_meter_data->step_of_qmax;
			else
				ret_Q_max = (iIndex - 1) * p_bat_meter_data->step_of_qmax; //ps:step_of_qmax=60
			break;
		}
	}

	printk("lwj--[fgauge_get_Q_max_by_current] i_curr(%d),i_Index(%d),OCV(%d),Rbat(%d),ret_Q_max(%d)\r\n",
...
void dod_init(void)
{
	...
	printk("lwj-[FGADC] get_hw_ocv=%d, HW_SOC=%d, SW_SOC = %d\n",
		 gFG_voltage, gFG_capacity_by_v, gFG_capacity_by_v_init);
...
/*add by lwj for test ---{*/
	temp_rtc=g_rtc_fg_soc;
	if(g_rtc_fg_soc>=0 && g_rtc_fg_soc < 4)
	{
		if( gFG_voltage<3590)
			g_rtc_fg_soc=4;
		else if(gFG_voltage<3610)
			g_rtc_fg_soc=5;
		else if(gFG_voltage<3640)
			g_rtc_fg_soc=6;		
		printk("lwj-- 0<=get_rtc_value(%d)<4,but vol=%d, and re-set g_rtc_fg_soc=%d\n",temp_rtc,gFG_voltage,g_rtc_fg_soc);
	}
/*add by lwj for test ---}*/
...
printk("lwj--%s: %d, %d, %d, %d\n", __func__, g_hw_ocv_debug, g_hw_soc_debug, g_sw_soc_debug,
	...
	printk("lwj-[FGADC] g_rtc_fg_soc=%d, gFG_capacity_by_v=%d\n",
		 g_rtc_fg_soc, gFG_capacity_by_v);
	...
	printk("lwj--gFG_capacity=gFG_capacity_by_c_init=gFG_capacity_by_c=%d,gFG_DOD1=%d\n",gFG_capacity,gFG_DOD1);
}
...
s32 fgauge_update_dod(void)
{
	...
	printk("lwj[fgauge_update_dod] FG_dod_1=%d, adjust_coulomb_counter=%d, gFG_columb=%d, gFG_DOD0=%d, gFG_temp=%d, gFG_BATT_CAPACITY=%d\r\n",
		 FG_dod_1, adjust_coulomb_counter, gFG_columb, gFG_DOD0, gFG_temp,
		 gFG_BATT_CAPACITY);	 
...
s32 fgauge_read_capacity(s32 type)
{
	...
	printk("lwj--%s--1,dvalue(FG_dod_1)=%d\n",__func__,dvalue);
...
//------------------{
//--test----{
void  bat_profile_tn_val_show(void)
{
	struct BATTERY_PROFILE_STRUCT *temp_profile_p;
	struct R_PROFILE_STRUCT *temp_profile_r_p;
	int i, saddles;

	temp_profile_p=fgauge_get_profile(p_bat_meter_data->temperature_t);
			
	saddles=p_bat_meter_data->battery_profile_saddles;
			
	/* Dumpt new battery profile */
	for (i = 0; i < saddles; i++) {
		printk("bat_pro_t: <DOD,Voltage> at %d = <%d,%d>\r\n",
			 p_bat_meter_data->temperature_t, (temp_profile_p + i)->percentage,
			 (temp_profile_p + i)->voltage);
	}

	
	temp_profile_r_p = fgauge_get_profile_r_table(p_bat_meter_data->temperature_t);
	/* Dumpt new r-table profile */
	for (i = 0; i < saddles; i++) {
		printk("r_pro_t:<Rbat,VBAT> at %d = <%d,%d>\r\n",
			 p_bat_meter_data->temperature_t, (temp_profile_r_p + i)->resistance,
			 (temp_profile_r_p + i)->voltage);
	}	
}
...
/* 1. Get Raw Data */
	...
	//add by lwj
	bat_Is_Charging=gFG_Is_Charging;
...
printk( "lwj-[FGADC] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
		 gFG_Is_Charging, gFG_current, gFG_columb, gFG_voltage, gFG_capacity_by_v,
		 gFG_capacity_by_c, gFG_capacity_by_c_init, gFG_BATT_CAPACITY,
		 gFG_BATT_CAPACITY_aging, gFG_compensate_value, gFG_ori_voltage,
		 p_bat_meter_data->ocv_board_compesate, p_bat_meter_data->r_fg_board_slope,
...
s32 battery_meter_trans_battery_percentage(s32 d_val)
{
	...
	printk("lwj[battery_meter_trans_battery_percentage] temp(%d),C_0mA(%d),C_600mA(%d),d_in(%d),d_o(%d),%d\r\n",
		 temp_val, C_0mA, C_600mA, d_val_before, d_val, g_currentfactor);
...
//----ad by lwj for test battery table 0505--{
char b_t_flag=2;
char r_t_flag=2;
static ssize_t show_bat_profile_tn_val(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct BATTERY_PROFILE_STRUCT *temp_profile_p;
	int i, saddles;
	char *ptr = buf;
	 ...
	 if(b_t_flag==0)
		temp_profile_p=p_bat_meter_data->p_battery_profile_t0;
	else if(b_t_flag==1)
		temp_profile_p=p_bat_meter_data->p_battery_profile_t1;
	else if(b_t_flag==2)
		temp_profile_p=p_bat_meter_data->p_battery_profile_t2;
	else if(b_t_flag==3)
		temp_profile_p=p_bat_meter_data->p_battery_profile_t3;
	else
		temp_profile_p=p_bat_meter_data->p_battery_profile_t2;
	...
	ptr +=sprintf(ptr, "bat_pro_t%d: <DOD,Voltage> at %d \r\n", b_t_flag,
			 g_fg_dbg_bat_temp);
		
	/* Dumpt new battery profile */
	for (i = 0; i < saddles; i++) {
		printk("bat_pro_t%d: <DOD,Voltage> at %d = <%d,%d>\r\n",b_t_flag,
			 g_fg_dbg_bat_temp, (temp_profile_p + i)->percentage,
			 (temp_profile_p + i)->voltage);
	...
	return (ptr-buf);//sprintf(buf, "%d\n", p_bat_meter_data->battery_profile_saddles);

}

static ssize_t store_bat_profile_tn_val(struct device *dev, struct device_attribute *attr,
				    const char *buf, size_t size)
{
	int get_val, ret;

	ret = kstrtoint(buf, 0, &get_val);
	if (ret) {
		pr_err("wrong format!\n");
		return size;
	}
...
static DEVICE_ATTR(bat_profile_tn_val, 0664,
		   show_bat_profile_tn_val, store_bat_profile_tn_val);

static ssize_t show_bat_id(struct device *dev, struct device_attribute *attr, char *buf)
{
	if(bat_id_pin_val==1)
		return sprintf(buf, "tcl\r\n");
	else
		return sprintf(buf, "hp\r\n");
	
	//return sprintf(buf, "bat_id=%d: [1:tcl ,0:hp] \r\n", bat_id_pin_val);
}
...
static DEVICE_ATTR(bat_id, 0664,
		   show_bat_id, store_bat_id);


static ssize_t show_charge_type(struct device *dev, struct device_attribute *attr, char *buf)
{
	
	return sprintf(buf, "charger type=%d \r\n", BMT_status.charger_type );
}

static ssize_t store_charge_type(struct device *dev, struct device_attribute *attr,
				    const char *buf, size_t size)
{
	return size;
}

static DEVICE_ATTR(charge_type, 0664,
		   show_charge_type, store_charge_type);


static ssize_t show_r_profile_tn_val(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	char *ptr = buf;
	 ...
	 if(r_t_flag==0)
		temp_profile_p=p_bat_meter_data->p_r_profile_t0;
	else if(r_t_flag==1)
		temp_profile_p=p_bat_meter_data->p_r_profile_t1;
	else if(r_t_flag==2)
		temp_profile_p=p_bat_meter_data->p_r_profile_t2;
	else if(r_t_flag==3)
		temp_profile_p=p_bat_meter_data->p_r_profile_t3;
	else
		temp_profile_p=p_bat_meter_data->p_r_profile_t2;
		
	saddles=p_bat_meter_data->battery_r_profile_saddles;
	
	ptr +=sprintf(ptr, "r_pro_t%d:<Rbat,VBAT> at %d \r\n", r_t_flag,
			 g_fg_dbg_bat_temp);
		
	/* Dumpt new r-table profile */
	for (i = 0; i < saddles; i++) {
		printk("r_pro_t%d:<Rbat,VBAT> at %d = <%d,%d>\r\n",r_t_flag,
			 g_fg_dbg_bat_temp, (temp_profile_p + i)->resistance,
			 (temp_profile_p + i)->voltage);
...
static ssize_t store_r_profile_tn_val(struct device *dev, struct device_attribute *attr,
				    const char *buf, size_t size)
{
	int get_val, ret;
...
static DEVICE_ATTR(r_profile_tn_val,0664,
		   show_r_profile_tn_val, store_r_profile_tn_val);
//-----
static ssize_t show_bat_profile_val(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct BATTERY_PROFILE_STRUCT *temp_profile_p;
	int i, saddles;
	char *ptr = buf;
	
	temp_profile_p=p_bat_meter_data->p_battery_profile_temperature;

	saddles=p_bat_meter_data->battery_profile_saddles;
	
	ptr +=sprintf(ptr, "<DOD,Voltage> at %d \r\n", 
			 g_fg_dbg_bat_temp);
...
static ssize_t store_bat_profile_val(struct device *dev, struct device_attribute *attr,
				    const char *buf, size_t size)
{
	return size;
}

static DEVICE_ATTR(bat_profile_val, 0664,
		   show_bat_profile_val, store_bat_profile_val);


static ssize_t show_r_profile_val(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct R_PROFILE_STRUCT *temp_profile_p;
	int i, saddles;
	char *ptr = buf;
	
	temp_profile_p=p_bat_meter_data->p_r_profile_temperature;

	saddles=p_bat_meter_data->battery_r_profile_saddles;
	
	ptr +=sprintf(ptr, "<Rbat,VBAT> at %d \r\n", 
			 g_fg_dbg_bat_temp);
...
static ssize_t store_r_profile_val(struct device *dev, struct device_attribute *attr,
				    const char *buf, size_t size)
{
	return size;
}
static DEVICE_ATTR(r_profile_val, 0664,
		   show_r_profile_val, store_r_profile_val);
//----ad by lwj for test battery table 0505--}
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	printk("lwj--%s--q_max_pos_50=%d\n",__func__,p_bat_meter_data->q_max_pos_50);
	printk("lwj--%s--rbat_pull_down_r=%d\n",__func__,p_bat_meter_data->rbat_pull_down_r);
	printk("lwj--%s--rbat_pull_up_r=%d\n",__func__,p_bat_meter_data->rbat_pull_up_r);
	printk("lwj--%s--fg_meter_resistance=%d\n",__func__,p_bat_meter_data->fg_meter_resistance);
	printk("lwj--%s--cust_r_sense=%d\n",__func__,p_bat_meter_data->cust_r_sense);
	printk("lwj--%s--r_fg_value=%d\n",__func__,p_bat_meter_data->r_fg_value);
	...
	if (get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT
		...
		mt65xx_leds_brightness_set(0, 255); //read on
	...
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_charge_type);	
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_bat_profile_tn_val);
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_r_profile_tn_val);	
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_bat_profile_val);
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_r_profile_val);
