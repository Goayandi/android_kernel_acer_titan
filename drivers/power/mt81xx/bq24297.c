...
#endif
	bq24297_set_iterm(0x0);	/* Termination current 0:128mA 1:256mA 2:384mA 5:768mA*/
...
static u32 charging_set_current(void *data)
{
	...
	printk("lwj--%s--set_current_value=%d--0\n",__func__,current_value);
...
static u32 charging_set_input_current(void *data)
{
	...
	printk("lwj--%s-set_chr_current=%d,register_value=%d--0\n",__func__,set_chr_current,register_value);
...
static u32 charging_set_hv_threshold(void *data)
{
	...
	printk("lwj--%s-set_hv_voltage=%d,register_value=%d--0\n",__func__,set_hv_voltage,register_value);
...
//printk("lwj--%s--0\n",__func__);
	...
	*(int *)(data) = STANDARD_HOST;
	...
	printk("lwj--charging_get_charger_type = %d\n", charger_type);
...
void bq24297_dump_register(void)
{
	...
	for (i = 0; i < bq24297_REG_NUM; i++) {
		...
		battery_log(BAT_LOG_FULL,
			...
			if(dump_count<2)
				printk("lwj--[bq24297_dump_register] Reg[0x%X]=0x%X\n", i, bq24297_reg[i]);
	}
...
{
	int ret = 0;
	//struct regulator *i2c_reg = devm_regulator_get(&client->dev, "reg-i2c"); /*Neostra huangjianlong 20170616*/

	pr_debug("[bq24297_driver_probe]\n");
	printk("lwj--%s--0\n",__func__);
	new_client = client;
	/*Neostra huangjianlong 20170616*/
/* 
	if (!IS_ERR(i2c_reg)) {
